...
export async function get({ params }) {
    let res = await supabase.from('coffee').select('name, producer, image, date, rating(rating, description, profile(name))').eq('id', params.id)
    if (res.error != null) {
        throw res.error;
    }
    let data = res.data[0];
...
export async function post({ request, params }) {
    ...
    let { data, error } = await supabase.from('rating').insert([
        { rating, description, coffee_id: params.id, profile_id: supabase.auth.user()?.id, }
    ])
